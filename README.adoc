...
The easiest way to get started with Smooks is to download and try out the https://github.com/smooks/smooks-examples/tree/v3[examples]. The examples are the recommended base upon which to integrate Smooks into your application.
// end::getting-started[]
...
Comparing the https://github.com/smooks/smooks/tree/v1.7.1/smooks-examples[code examples] for Smooks 1 with https://github.com/smooks/smooks-examples/tree/v3[those for Smooks 2] can be a useful guide in migrating to Smooks 2. While not exhaustive, we have compiled a list of notes to assist your migration:
...
. Replace `closeSource` attribute in the XML config element `core:filterSettings` with `closeSink`.
...
. Follow the https://github.com/smooks/smooks-examples/tree/v3/edifact-to-java[EDIFACT-to-Java example] to migrate an implementation that binds an EDIFACT document to a POJO.
. Follow the https://github.com/smooks/smooks-examples/tree/v3/java-to-edifact[Java-to-EDIFACT example] to migrate an implementation that deserialises a POJO into an EDIFACT document.
...
TIP: The full example is https://github.com/smooks/smooks-examples/tree/v3/pipelines[available in the smooks-examples repository].
...
The CSV and Regex readers are implemented using these abstract classes. See the https://github.com/smooks/smooks-examples/tree/v3/csv-variable-record[csv-variable-record] and https://github.com/smooks/smooks-examples/tree/v3/flatfile-to-xml-regex[flatfile-to-xml-regex] examples. The https://github.com/smooks/smooks-examples/tree/v3/flatfile-to-xml-regex[Regex Reader] implementation is also a good example that can be used as a basis for your own custom flat file reader.
